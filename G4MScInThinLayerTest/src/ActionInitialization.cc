...
// LHCb
ActionInitialization::ActionInitialization(DetectorConstruction* det, RMSPlotter* rmsplotter)
 : G4VUserActionInitialization(),fDetector(det),fRMSPlotter(rmsplotter)
{ }
...
void ActionInitialization::Build() const
{
  ...
  // LHCb
  RunAction* runaction = new RunAction(fDetector, primary, fRMSPlotter);
  SetUserAction(runaction);

  EventAction* eventaction = new EventAction();
  SetUserAction(eventaction);

  SetUserAction(new TrackingAction(fDetector, eventaction, fRMSPlotter));
